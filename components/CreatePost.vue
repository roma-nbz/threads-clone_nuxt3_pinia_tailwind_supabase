...
const createPost = async () => {
	...
	if (fileData.value) {
		const { data, error } = await client.storage.from('threads-file').upload(`${uuidv4()}.jpg`, fileData.value)
	...
	<div id="CreatePost" class="fixed z-50 bottom-0 h-full w-full overflow-hidden">
		<div class="bg-black h-full w-full text-white overflow-auto">
			<div class="flex items-center justify-start py-4 max-w-[500px] mx-auto border-b border-b-gray-700">
				...
				<p class="text-[16px] font-semibold">New Thread</p>
			</div>
				...
				<div class="py-2 w-full">
					<div class="flex items-center">
						<div v-if="user" class="flex items-center text-white">
							...
							<p class="ml-2 font-semibold text-[18px]">{{ user.identities[0].identity_data.full_name }}</p>
						</div>
					</div>
					<div class="relative flex items-center w-full">
						...
						<div class="bg-black rounded-lg w-[calc(100%-50px)] font-light">
							<div class="pt-2 text-gray-300 bg-black w-full">
								<textarea class="w-full bg-black outline-none" v-model="text" style="resize: none"
							...
							<div class="w-full">
								<div class="flex flex-col gap-2 py-2">
									<div v-if="fileDisplay">
										<img class="mx-auto w-full my-2 mr-2 rounded-lg" :src="fileDisplay" alt="">
									</div>
									<label for="fileInput">
